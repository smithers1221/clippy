...
module.exports = function(controller) {
    ...
    controller.hears(['food', 'hungry', 'lunch', 'truck'], 'direct_message,direct_mention,mention', function(bot, message) {
        ...
        controller.storage.users.get(message.user, function(err, user) {
            ...
            T.get('statuses/user_timeline', { count:20, since_id: moment().utcOffset(-8).add(-1, 'days').format("YYYY-MM-DD"), trim_user: true, exclude_replies: true, user_id: 2295568387 }, function(err, data, response) {
                var found = false;
                for (var i = 0; i < data.length; i++) {
                    if ((data[i].text.indexOf('@bLAckwelder_LA') != -1) && (data[i].text.indexOf(moment().utcOffset(-8).format('ddd').toUpperCase()) != -1)) {
